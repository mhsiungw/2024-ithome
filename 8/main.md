...
EC2 依照 Instance 部署的地點分為三種不同的 Tenancy Types，分別是 Shared Tenancy、Dedicated Instance 與 Dedicated Host
...
Dedicated Instance 讓你可以獨自享有硬體，不必與其他帳戶共享，但當 Instance 重新啟動時，不保證會在同一台機器上。在定價上，以 t3.nano 舉例，一小時是 $0.0052 美金，但因為 Dedicated Instance 的關係，每個 Region 只要有跑 Dedicated Instance，就會再加收 $2 美金，但這個價錢是以 Region 為單位計價，而不是 Instance 的數量。
...
Dedicated Instance 與 Dedicated Host 都能夠讓你不必與其他帳戶共用硬體，那到底該選哪一種呢？
因為 Dedicated Host 可以讓你選擇你要在哪一台機器上部署 Instance，且也可以設定當 Instance 重啟時，是否要在同一台機器上，如果這兩個特性是你需要的，就選擇 Dedicated Host 吧！
...
Spot Instance 讓你可以透過使用 AWS 目前未被使用的運算資源，節省高達 90% 的費用，但缺點是 AWS 可以隨時停止服務。
